...
| Table scope | `public.*` | Supports `schema.table`, comma-separated lists, and `schema.*`. |
| Source table prefix | empty | Use when source physical tables are prefixed, such as `edu_user`. |
| Target table prefix | empty | Use when target physical tables are prefixed, such as `edu_user`. |
